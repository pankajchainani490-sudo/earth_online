...
如果网络下载 Gradle 超时或受限，可以直接使用本地缓存的 Gradle：
...
2. 找到已下载完成的版本目录（包含 `gradle-x.x/bin/gradle`）：
```bash
ls ~/.gradle/wrapper/dists/gradle-8.9-bin/d3x0641q7zvyxj470aijz0e22/gradle-8.9/bin/
```

3. 直接使用该路径的 Gradle：
```bash
~/.gradle/wrapper/dists/gradle-8.9-bin/d3x0641q7zvyxj470aijz0e22/gradle-8.9/bin/gradle -p android assembleDebug
```

#### 如何找到本地 Gradle 缓存

Gradle 包装器下载的 Gradle 版本存储在 `~/.gradle/wrapper/dists/` 目录下。
...
# 2. 检查某个版本是否下载完成（看是否有 .ok 文件）
ls -la ~/.gradle/wrapper/dists/gradle-8.9-bin/*/

# 3. 确认 bin 目录存在则可用
ls ~/.gradle/wrapper/dists/gradle-8.9-bin/*/gradle-8.9/bin/
```

**判断版本是否可用**：
- 有 `.ok` 文件 + `gradle-x.x` 目录 → 可用
- 只有 `.part` 文件 → 下载未完成
- 只有 `.lck` 文件 → 正在下载或被锁定
